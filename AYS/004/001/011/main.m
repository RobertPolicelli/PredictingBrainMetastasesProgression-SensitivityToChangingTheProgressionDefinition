...
[vdARECalls, vdRNCalls] = FileIOUtils.LoadMatFile(...
    fullfile(ExperimentManager.GetPathToExperimentAssetResultsDirectory('AYS-004-001-002'), '01 Analysis', 'Outcomes Per Sample.mat'),...
    'vbIsAdverseRadiationEffectPerSample', 'vbIsRadionecrosisPerSample');
vdKnownCalls = logical(vdARECalls + vdRNCalls);

dNumberOfSamples = length(vdPatientIds);

vsTrajectoryPerSample = strings(dNumberOfSamples,1);
vbNoFollowUps = false(dNumberOfSamples,1);
for dSampleIndex=1:dNumberOfSamples
    ...
    vbKeepFollowUp = vdFollowUpRadiologyAssessmentTimeFromTreatment_months <= 15; % disregard past 1.25 years
    
    voFollowUpRadiologyAssessments = voFollowUpRadiologyAssessments(vbKeepFollowUp);  
    if isempty(voFollowUpRadiologyAssessments)
        vbNoFollowUps(dSampleIndex) = true;
    end
...
vbSampleSelect = vdPatientIds == 35 & vdBMNumbers == 3;

vbIsProgressionPerSample(vbSampleSelect) = false;
vbIsAdverseRadiationEffectPerSample(vbSampleSelect) = true;
...
vbIsProgressionPerSample(vdKnownCalls) = false;
vbIsRadionecrosisPerSample(vdRNCalls) = true;
vbIsAdverseRadiationEffectPerSample(vdARECalls) = true;
...
% otherwise, the relative 25% measure is used.
    ...
    if dVolume1 <= 108 || dVolume2 <= 108 % then (3mm)^3 minimum change is used
        ...
        if dPercentChange >= 0.25
            dChange = +1;
        elseif dPercentChange <= -0.25
            dChange = -1;
        else
            dChange = 0;
        end

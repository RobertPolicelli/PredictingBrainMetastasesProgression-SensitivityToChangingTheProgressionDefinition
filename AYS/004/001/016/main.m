...
for dSampleIndex=1:dNumberOfSamples
    ...
    vbKeepFollowUp = vdFollowUpRadiologyAssessmentTimeFromTreatment_months <= 24; % disregard past 1 years
...
function dAboveValue = ComputeValueChange(dVolume1, dVolume2)
    if dVolume1 <= 108 || dVolume2 <= 108 % then 3mm minimum change is used
        if dVolume2 - dVolume1 >= -27
            dAboveValue = +1;
        elseif dVolume2 - dVolume1 < -27
            dAboveValue = -1;

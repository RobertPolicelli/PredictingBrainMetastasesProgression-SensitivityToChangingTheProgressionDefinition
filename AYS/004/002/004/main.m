...
for dSampleIndex=1:dNumberOfSamples
    ...
    if (vdRANOIncludingBaseline(end) == 0)
        vbCR(dSampleIndex) = true;
    end
...
% " CR"
vbIsProgressionPerSample(vbCR == true) = false;
...
function dAboveValue = ComputeValueChange(dDiameter1, dDiameter2)
    if dDiameter1 <= 10 || dDiameter2 <= 10 % then 3mm minimum change is used
        if dDiameter2 - dDiameter1 >= -3
            dAboveValue = +1;
        elseif dDiameter2 - dDiameter1 < -3

...
for dSampleIndex=50:dNumberOfSamples
...
% %%%%%%%%%%%%%%%%%%
% "+-+-": decrease then increase then decrease then increase 15 RN
%+-+ 24 PR
...
% otherwise, the relative 25% measure is used.
    ...
    if dVolume1 <= 270 || dVolume2 <= 270 % then (3mm)^3 minimum change is used
        ...
        if dPercentChange >= 0.10
            dChange = +1;
        elseif dPercentChange <= -0.10
            dChange = -1;
        else
            dChange = 0;
        end

...
% otherwise, the relative 25% measure is used.
    ...
    if dVolume1 <= 180 || dVolume2 <= 180 % then (3mm)^3 minimum change is used
        ...
        if dPercentChange >= 0.15
            dChange = +1;
        elseif dPercentChange <= -0.15
            dChange = -1;
        else
            dChange = 0;
        end
...
function dAboveValue = ComputeValueChange(dVolume1, dVolume2)
    if dVolume1 <= 180 || dVolume2 <= 180 % then 3mm minimum change is used
        if dVolume2 - dVolume1 >= -27
            dAboveValue = +1;
        elseif dVolume2 - dVolume1 < -27
            dAboveValue = -1;

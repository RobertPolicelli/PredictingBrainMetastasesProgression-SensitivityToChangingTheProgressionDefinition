classdef DicomRTStructClosedPolygonStructure < matlab.mixin.Copyable
    ...
    methods (Access = public)
        ...
        function obj = DicomRTStructClosedPolygonStructure(stFileMetadata, dContourNumber, oImageVolumeGeometry)
            ...
            if ~isfield(stFileMetadata.ROIContourSequence.(chItemField), 'ContourSequence')
                ...
                c1oClosedPlanarPolygons = cell(dNumPolygons, 1);
                vbIsEmpty = false(dNumPolygons,1);
                
                for dPolygonIndex=1:dNumPolygons
                    c1oClosedPlanarPolygons{dPolygonIndex} = DicomRTStructClosedPolygonStructure.CreateClosedPlanarPolygonFromContourSequenceMetadata(stContourSequence, dPolygonIndex, oImageVolumeGeometry);
                    vbIsEmpty(dPolygonIndex) = isempty(c1oClosedPlanarPolygons{dPolygonIndex});
                end
                
                c1oClosedPlanarPolygons = c1oClosedPlanarPolygons(~vbIsEmpty);
                voClosedPlanarPolygons = CellArrayUtils.CellArrayOfObjects2MatrixOfObjects(c1oClosedPlanarPolygons);
    ...
    methods (Access = private, Static = true)
        function oPolygon = CreateClosedPlanarPolygonFromContourSequenceMetadata(stContourSequence, dPolygonIndex, oImageVolumeGeometry)
            ...
            if ~isnumeric(vdCoords_mm)
                oPolygon = ClosedPlanarPolygon.empty;
                
                warning(...
                    'DicomRTStructLabelMapRegionsOfInterest:Constructor:InvalidPolygonData',...
                    'CLOSED_PLANAR polygon RT Struct contained non-numeric data so the polygon was ignored.');
            else

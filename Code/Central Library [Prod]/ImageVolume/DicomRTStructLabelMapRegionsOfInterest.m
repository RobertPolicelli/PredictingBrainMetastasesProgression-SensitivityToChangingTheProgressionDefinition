classdef DicomRTStructLabelMapRegionsOfInterest < LabelMapRegionsOfInterest & RegionsOfInterestFromPolygons
    ...
    methods (Access = public)
        ...
        function obj = DicomRTStructLabelMapRegionsOfInterest(chFilePath, oDicomImageVolume)
            ...
            try
                lastwarn('',''); % clear out last warning
                stFileMetadata = dicominfo(chFilePath);
                
                if ~isempty(lastwarn) % try again with 'UseDictionaryVR' = true
                    lastwarn('',''); % clear out last warning
                    stFileMetadata = dicominfo(chFilePath, 'UseDictionaryVR', true);
                    
                    if ~isempty(lastwarn) % try again with 'UseVRHeuristic' = false
                        lastwarn('',''); % clear out last warning
                        stFileMetadata = dicominfo(chFilePath, 'UseVRHeuristic', false);
                        
                        if ~isempty(lastwarn) % try again with 'UseVRHeuristic' = false and 'UseDictionaryVR' = true
                            lastwarn('',''); % clear out last warning
                            stFileMetadata = dicominfo(chFilePath, 'UseDictionaryVR', true, 'UseVRHeuristic', false);
                            
                            if ~isempty(lastwarn)
                                error(...
                                    'DicomRTStructLabelMapRegionsOfInterest:Constructor:dicominfoFailure',...
                                    'Warning issued during dicominfo');
                            end
                        end
                    end
                end
        ...
        function SelectRegionsOfInterest(obj, vdRegionOfInterestNumbers)
            ...
            if any(obj.vdSelectedRegionsOfInterest' ~= 1:obj.dNumberOfRegionsOfInterestInFile)
                error(...
                    'DicomRTStructLabelMapRegionsOfInterest:SelectRegionsOfInterest:AlreadySelected',...
                    'ROIs can only be selected once.');                
            end
            ...
            m3uiUpdatedLabelMaps = zeros(size(m3uiLabelMaps), obj.GetLabelMapUintType(obj.GetNumberOfRegionsOfInterest()));
            
            for dRegionOfInterestNumber=1:obj.GetNumberOfRegionsOfInterest()
                m3uiUpdatedLabelMaps = bitset(m3uiUpdatedLabelMaps, dRegionOfInterestNumber, bitget(m3uiLabelMaps, obj.vdSelectedRegionsOfInterest(dRegionOfInterestNumber)));
            end
            
            obj.m3uiLabelMaps = m3uiUpdatedLabelMaps;
        end

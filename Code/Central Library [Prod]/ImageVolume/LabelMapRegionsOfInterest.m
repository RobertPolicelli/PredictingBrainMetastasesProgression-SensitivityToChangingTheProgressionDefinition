...
%     end
    ...
    methods (Access = public)
        ...
        function SaveToNIfTI(obj, sFilePath)            
            arguments
                obj (1,1) LabelMapRegionsOfInterest
                sFilePath (1,1) string
            end
            
            % label maps in Nifti need to be stored a little differently
            % than the label maps. In the label maps class, ROIs are
            % allowed to overlap (e.g. one voxel can be in more than one
            % class) and this is done using a bit map. In a Nifti label map
            % file, each voxel can only be part of a single ROI and
            % therefore the voxel value contains the ROI number (or 0 if
            % not part of any ROI). This will be validated against as the
            % labelmap matrix is generated.
            
            dNumROIs = obj.GetNumberOfRegionsOfInterest();  
            
            if dNumROIs > 255
                error(...
                    'LabelMapRegionsOfInterest:SaveToNIfTI:TooManyROIs',...
                    'A maximum of 255 regions of interest can be saved to a NIfTI file.');
            end
            
            oImageVolumeGeometry = obj.GetImageVolumeGeometry();
            
            vdDims = oImageVolumeGeometry.GetVolumeDimensions();
            
            m3uMaskData = zeros(vdDims, 'uint8');
            
            for dROINumber=1:dNumROIs
                m3bMask = obj.GetMaskByRegionOfInterestNumber(dROINumber);
                
                if any(m3uMaskData(m3bMask) ~= 0)
                    error(...
                        'LabelMapRegionsOfInterest:SaveToNIfTI:OverlappingROIs',...
                        'ROIs cannot overlap when being saved to a NIfTI file.');
                end
                
                m3uMaskData(m3bMask) = dROINumber;
            end
            
            NIfTIImageVolume.WriteDataMatrixToNIfTIFile(m3uMaskData, oImageVolumeGeometry, sFilePath);
        end

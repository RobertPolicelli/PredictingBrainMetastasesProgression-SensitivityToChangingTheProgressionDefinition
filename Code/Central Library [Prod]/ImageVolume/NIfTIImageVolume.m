classdef NIfTIImageVolume < ImageVolume
    ...
    methods (Access = {?NIfTILabelMapRegionsOfInterest, ?ImageVolume, ?LabelMapRegionsOfInterest}, Static = true)
        ...
        function oImageVolumeGeometry = GetImageVolumeGeometryFromFileMetaData(stFileMetaData, NameValueArgs)
            ...
            if stFileMetaData.raw.qform_code == 0
                error(...
                    'NIfTIImageVolume:GetImageVolumeGeometryFromFileMetaData:InvalidQFormCode',...
                    'Q transform not defined.');
            else
                % refer to:
        ...
        function WriteDataMatrixToNIfTIFile(m3xImageData, oImageVolumeGeometry, sFilePath)
            
            m2dRotationMatrix = [...              
                oImageVolumeGeometry.GetRowAxisUnitVector',... 
                oImageVolumeGeometry.GetColumnAxisUnitVector',...
                oImageVolumeGeometry.GetSliceAxisUnitVector'];
            
            m2dAffineMatrix = [m2dRotationMatrix', [0 0 0]'; oImageVolumeGeometry.GetFirstVoxelPosition_mm(), 1];
            oTransform = affine3d(m2dAffineMatrix);
            
            stMeta = struct(...
                'Version', 'NIfTI1',...
                'ImageSize', oImageVolumeGeometry.GetVolumeDimensions(),...
                'PixelDimensions', oImageVolumeGeometry.GetVoxelDimensions_mm(),...
                'Description', '',...
                'Datatype', class(m3xImageData),...
            ...
            niftiwrite(m3xImageData, sFilePath, stMeta);
        end
    end

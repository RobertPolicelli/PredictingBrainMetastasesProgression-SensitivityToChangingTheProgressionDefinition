classdef (Abstract) RegionsOfInterest < GeometricalImagingObject
    ...
    methods (Access = public, Static = true)
        ...
        function obj = Load(chMatFilePath)
            ...
            vstVariablesInMatfile = whos('-file', chMatFilePath);
            dNumVariables = length(vstVariablesInMatfile);
            
            vsVariablesInMatFile = strings(dNumVariables,1);
            
            for dVarIndex=1:dNumVariables
                vsVariablesInMatFile(dVarIndex) = string(vstVariablesInMatfile(dVarIndex).name);
            end
            
            if ~any(vsVariablesInMatFile == string(RegionsOfInterest.chObjMatFileVarName))
                error(...
                    'RegionsOfInterest:Load:InvalidMatFile',...
            ...
            obj.vsVariablesInMatFile = vsVariablesInMatFile;
    ...
    methods (Access = ?ImageVolume, Static = true)
        ...
        function bBool = IsValidMatFilePath(chMatFilePath)            
            bBool = ~isempty(whos('-file', chMatFilePath, RegionsOfInterest.chObjMatFileVarName));
        end
    end

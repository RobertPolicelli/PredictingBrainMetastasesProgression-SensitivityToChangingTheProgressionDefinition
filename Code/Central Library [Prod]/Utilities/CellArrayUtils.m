classdef (Abstract) CellArrayUtils
    ...
    methods (Access = public, Static = true)
        ...
        function bBool = AreEqual(cNxCellArray1, cNxCellArray2)
            ...
            if length(dDims1) == length(dDims2) && all(dDims1 == dDims2)
                for dIndex=1:numel(cNxCellArray1)
                    dCellDims1 = size(cNxCellArray1{dIndex});
                    dCellDims2 = size(cNxCellArray2{dIndex});
                    
                    if length(dCellDims1) == length(dCellDims2) && all(dCellDims1 == dCellDims2)
                        if ~all(cNxCellArray1{dIndex} == cNxCellArray2{dIndex})
                            bBool = false;
                            break;
                        end                        
                    else
        ...
        function mXxMatrix = CellArrayOfObjects2MatrixOfObjects(cXoObjects)
            ...
            if isempty(cXoObjects)
                mXxMatrix = [];
            else            
                mXxMatrix = repmat(cXoObjects{1},size(cXoObjects));
                
                for dElementIndex=1:numel(cXoObjects)
                    if isempty(cXoObjects{dElementIndex})
                        error(...
                            'CellArrayUtils:CellArrayOfObjects2MatrixOfObjects:EmptyValue',...
                            'Cell array cannot contain empty values when being converted into a matrix of objects.');
                    else

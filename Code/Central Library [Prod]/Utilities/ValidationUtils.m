classdef (Abstract) ValidationUtils
    ...
    methods (Access = public, Static = true)
        ...
        function MustBeValidRgbVector(vdVector_rgb)
            ...
            arguments
                vdVector_rgb (:,3) double {mustBeNonnegative(vdVector_rgb), mustBeLessThanOrEqual(vdVector_rgb,1)}
            end
        end
        ...
        function MustBeInOrder(vxData, fnOrderMetric, sOrderDirection)
            ...
            if ~isempty(vxData)
                if isempty(fnOrderMetric)
                    ...
                    vbEmptyMetricValue = false(dNumElements,1);
                    
                    for dIndex=1:dNumElements
                        c1xSortMetricValues{dIndex} = fnOrderMetric(vxData(dIndex));
                        vbEmptyMetricValue(dIndex) = isempty(c1xSortMetricValues{dIndex});
                    end
                    
                    [~,vdSortIndices] = sort(CellArrayUtils.CellArrayOfObjects2MatrixOfObjects(c1xSortMetricValues(~vbEmptyMetricValue)), sOrderDirection); % ignore empty values
                end
                                
                if ~all(vdSortIndices == (1:length(vdSortIndices))')
                    error(...
